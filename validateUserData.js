...
function validateUserData(userData) {
    ...
    if (!userData.password) {
        ...
        if (!/\d/.test(userData.password)) {
            result.isValid = false;
            result.errors.password = (result.errors.password || "") + " Password must contain at least one number.";
        }
        if (!/[!@#$%^&*(),.?":{}|<>]/.test(userData.password)) {
            result.isValid = false;
            result.errors.password = (result.errors.password || "") + " Password must contain at least one special character.";
        }
    }
